...
Notifications include UUID id, content, ordered actions, optional callbacks, creation/update timestamps (Unix seconds), status, readAt, revision, delivery state, registration state, and a typed response. AgentNotify records accepted sends as `accepted` with `nativeRegistered: false` and does not register or deliver them through UserNotifications. Responses distinguish action/reply/body/close/timeout/interrupt and track callback effect running/succeeded/failed/interrupted.
...
The `ui` operations control semantic AppKit/SwiftUI state rather than screen coordinates. `uiState` returns an app-launch `instanceId`, in-memory `uiRevision`, visible surfaces, popover/panel and pin state, current category/search/group/period, exact selection, up to 100 compact matching rows, custom arrival state, and the current Undo target. A non-null Undo target includes `items` (exact IDs and expected revisions) and `count`; a single-item target also retains the legacy top-level `id` and `expectedRevision` fields. Reading this state and selecting or navigating never marks a notification read.

`uiShow` opens the inbox or Preferences; inbox calls can reveal an exact notification and set `pinned`. `uiClose` closes the inbox, Preferences, arrival, or all surfaces. `uiSetView` changes filters, search, selection, and detail expansion atomically. `uiNavigate` selects first/previous/next/last without wrapping or reading. `uiSetPinned` pins or unpins, `uiDismissArrival` closes only the custom arrival, and `uiCopy` copies the selected or exact notification's text or ID. The compact arrival's Complete control uses the durable `status done` behavior for its displayed ID: an unanswered prompt receives a close response without executing a callback, remaining burst content advances, and the surface dismisses after the final item.
...
`preferences` returns `{arrivalStyle, showBannerReminder, completeAllShortcut, revision}`; a new store returns `queue-peek`, `showBannerReminder: false`, the default `⌥⇧⌘D` shortcut (`{keyCode: 2, key: "D", modifiers: ["option", "shift", "command"]}`), and revision 1. `showBannerReminder` is a deprecated, visually inert compatibility field; it cannot enable macOS presentation. `setPreferences` accepts at least one of `arrivalStyle` (`queue-peek`, `compact-toast`, or `queue-shelf`), the retained compatibility field, or `completeAllShortcut`. A shortcut is either `null` to clear it or `{keyCode, key, modifiers}`, where `keyCode` is 0–127, `key` is its display label, and `modifiers` contains two to four unique values from `control`, `option`, `shift`, and `command`. The shortcut opens the same count-aware Complete All confirmation as the native menu; it never completes notifications silently. `expectedRevision` and `requestId` remain optional. Omitted settings retain their current values, and a multi-setting change is one atomic revision. Writes are durable and transactional. Preferences are local to this inbox service and separate from both change feeds.
...
`shimStatus` returns installed/available booleans, the target and installer paths, `originalNotifier` (the Homebrew-linked original path, or null), and `promptHandled`. `installShim` delegates to the existing `~/code/agentstart/scripts/install-notification-shim` owner, then durably handles the setup offer. This requires AgentStart and explicit human intent; it installs the router into `~/.local/bin` and refuses foreign commands. It fails with `original_notifier_present` when Homebrew `terminal-notifier` is still linked at `/opt/homebrew/bin` or `/usr/local/bin`. It does not edit shell profiles or uninstall Homebrew. `dismissShimSetup` durably handles the offer without installation. All three are CLI/socket/MCP peers, emit no notification changes, and remain separate from arrival-style revisions. An isolated `AGENTNOTIFY_STATE_DIR` also isolates the shim destination beneath `<state>/shim-home` to protect the real PATH during tests, and does not inspect the operator's Homebrew prefixes.
