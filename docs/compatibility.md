...
- With macOS banners disabled, the GUI also presents one compact, nonactivating arrival preview. It stays until dismissed or opened, updates to the latest arrival without stacking panels, and summarizes unread notifications plus read items that still need attention. Preview dismissal is not a terminal-notifier close response. Clicking it reads the displayed item without executing its body callback. Enabled system banners keep their original behavior. Native `delivery` and legacy exit codes continue to describe native delivery, not this additional inbox projection.
