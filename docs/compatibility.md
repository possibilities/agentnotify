...
- With macOS banners disabled, the GUI also presents a compact, nonactivating arrival preview. It disappears after five seconds or can open the full inbox; this dismissal is not a terminal-notifier close response. Clicking it reads the item without executing its body callback. Enabled system banners keep their original behavior. Native `delivery` and legacy exit codes continue to describe native delivery, not this additional inbox projection.
