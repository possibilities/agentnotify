...
- The GUI always presents its compact, nonactivating arrival preview. It disappears after five seconds or can open the full inbox; this dismissal is not a terminal-notifier close response. Clicking it reads the item without executing its body callback. Optional system banners may appear alongside it when enabled. A system banner body click now opens the item in AgentNotify without executing its callback; explicit native action and reply controls retain their response behavior. Native `delivery` and legacy exit codes continue to describe native delivery, not the AgentNotify arrival projection.
