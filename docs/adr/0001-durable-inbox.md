...
SQLite WAL is the source of truth; system notifications are a projection. One account-local app owns a private Unix socket, transactions, native delivery, and callback execution. The CLI and stdio MCP call the same service. A headless service exists for isolated tests and non-GUI use.
...
Use SwiftUI/AppKit rather than a cross-platform rendering layer: this product depends on macOS menu bar, popover, panel, accessibility, and UserNotifications behavior. Keep presentation monochrome and tied to system appearance. The popover and detached panel share the same view and state.
