...
with tempfile.TemporaryDirectory(prefix='an-', dir='/tmp') as tmp:
    ...
    try:
        ...
        check({t['name'] for t in tools} == {'send','list','get','status','statusBatch','respond','remove','changes','diagnose','show','heartbeat','preferences','setPreferences','showPreferences','shimStatus','installShim','dismissShimSetup','uiState','uiShow','uiClose','uiSetView','uiNavigate','uiSetPinned','uiDismissArrival','uiCopy'}, 'MCP parity catalog')
