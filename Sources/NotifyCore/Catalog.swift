...
public enum Catalog {
    ...
    public static let operations: [Operation] = [
        ...
        Operation(name: "shimStatus", summary: "Check terminal-notifier PATH shim installation, whether a Homebrew original is still linked, and whether the one-time setup offer was handled. Sends no notification.", mutates: false, parameters: []),
        Operation(name: "installShim", summary: "Install the terminal-notifier router in ~/.local/bin using the fleet's AgentStart installer. Requires explicit human intent. Refuses if Homebrew terminal-notifier is still linked, refuses foreign commands, and does not edit shell profiles.", mutates: true, parameters: []),
